...
from core.analyzer import DNASilverAnalyzer, MAX_SILVER, MAX_STRANDS
...
# Try to import IsoSpecPy for faster isotope pattern generation
try:
    import IsoSpecPy as isospec
    ISOSPEC_AVAILABLE = True
    logger.info("IsoSpecPy loaded - faster isotope pattern generation available")
except ImportError:
...
def check_rate_limit(ip_address: str | None) -> bool:
    ...
    if ip_address in _rate_limit_requests:
        _rate_limit_requests[ip_address] = [
            t for t in _rate_limit_requests[ip_address]
...
def validate_numeric_param(
    value: Any, min_val: float, max_val: float, name: str
) -> tuple[bool, str | None]:
...
# Security headers middleware (disabled for performance testing - uncomment to enable)
# @app.after_request
...
#         "default-src 'self'; "
#         "script-src 'self' 'unsafe-inline' 'unsafe-eval' https://peter-ertl.com; "
#         "style-src 'self' 'unsafe-inline'; "
#         "img-src 'self' data: blob:; "
#         "font-src 'self'; "
#         "connect-src 'self'; "
#         "frame-src https://peter-ertl.com;"
#     )
#     return response
...
def add_manual_composition_by_formula() -> FlaskResponse:
    ...
    try:
        ...
        resolution = int(data.get('resolution', 20000))
        ...
        if custom_xna and custom_xna.get('formula') and custom_xna.get('molecular_weight') is not None and num_phosphorus > 0:
            ...
            if p_per_strand > 0:
                ...
                if abs(estimated_strands_raw - estimated_strands) > 0.1:
                    logger.warning(f"P count ({num_phosphorus}) doesn't divide evenly by P per strand ({p_per_strand}). Calculated: {estimated_strands_raw:.2f} strands → Rounded to: {estimated_strands}. The entered formula may not match the XNA composition!")
                ...
                logger.info(f"XNA mode - estimated {estimated_strands} strands from P count {num_phosphorus}, XNA formula: {custom_xna['formula']} (P={p_per_strand}), Total neutral mass: {user_neutral_mass:.2f} Da")
            else:
                logger.warning(f"XNA formula has no P atoms, cannot estimate num_strands")
        ...
        if 'error' not in theo_pattern:
            ...
            if provided_exp_x0 is not None:
                ...
                logger.debug(f"[add_manual_composition_by_formula] Using provided exp_x0 = {exp_x0:.4f} from manual fit")
        ...
        composition = {
            ...
            'nH': 0, 'nC': 0, 'nN': 0, 'nO': 0, 'nP': 0,  # Not parsed individually
...
def add_manual_composition() -> FlaskResponse:
    ...
    try:
        ...
        adduct_formula_parts = []
        adduct_elements: dict[str, int] = {}  # For ion formula: {element: count}

        if adducts_input:
            logger.info("Processing adducts for custom composition")
            for adduct_entry in adducts_input:
                ...
                logger.debug(f"Adduct: {adduct_count}×{adduct_name}: mass={adduct_mass * adduct_count:.4f} Da, charge={adduct_charge * adduct_count:+d}")

            # Build combined adduct string (e.g., "NH4+2Na+Cl")
            adduct_string = '+'.join(adduct_formula_parts) if adduct_formula_parts else ''
            logger.info(f"Total adduct: {adduct_string} (mass={total_adduct_mass:.4f} Da, charge={total_adduct_charge:+d})")
        else:
            adduct_string = ''
        ...
        if custom_xna and custom_xna.get('formula'):
            ...
            if is_complex_mode and strand_type in ['strand1', 'strand2']:
                ...
                strand2_formula = custom_xna.get('strand2_formula', '') or strand1_formula  # Fallback to strand1 if same strands
            ...
            logger.info(f"XNA mode - using custom molecular weight: {xna_formula}, Total neutral mass: {user_neutral_mass:.2f} Da")
...
def add_manual_composition_search() -> FlaskResponse:
    ...
    try:
        ...
        logger.info(f"SEARCH MODE: Finding best N₀ for specified adducts - nf={num_strands}, nAg={num_silver}, Peak: m/z={peak_mz:.4f}, z={charge}")
        ...
        # Process multiple adducts with counts
        total_adduct_mass = 0.0
        total_adduct_charge = 0
        ...
        if adducts_input:
            logger.info("Processing adducts for search mode")
            for adduct_entry in adducts_input:
                adduct_name = adduct_entry.get('name')
                adduct_count = int(adduct_entry.get('count', 1))
        ...
        adduct_string = '+'.join(adduct_formula_parts) if adduct_formula_parts else ''
        if adduct_string:
            logger.info(f"Total adducts: {adduct_string} (mass={total_adduct_mass:.4f} Da, charge={total_adduct_charge:+d})")
        else:
            logger.debug("No adducts specified")
        ...
        logger.info(f"Best composition (by X₀ error): N₀={true_best['n0']}, Qcl={true_best['qcl']}, X₀ error: {true_best['x0_error']:.4f} m/z, Pattern score: {true_best['pattern_score']:.3f}")
        ...
        if is_complex:
            ...
            if complex_comp:
                logger.info(f"COMPLEX MODE: Returning only N₀=0 composition - nAg={complex_comp['num_silver']}, Qcl={complex_comp['qcl']}, X₀_err={complex_comp['x0_error']:.4f}")
...
def reanalyze_peak() -> FlaskResponse:
    ...
    try:
        ...
        compositions = analyzer.analyze_peak_with_smart_adduct_search(
            peak_mz, charge, dna_sequence, exp_x0,
        ...
        if len(compositions) > 0:
            ...
            refined_compositions, _, _, has_other_strands, all_compositions, has_odd_n0_warning, _, _, has_unrealistic_n0_warning = analyzer.refine_compositions_with_isotope_matching(
...
def recalculate_peak_with_manual_fit() -> FlaskResponse:
    ...
    try:
        ...
        logger.info(f"USER_ACTION: Manual Gaussian fit - Peak m/z: {peak_mz:.4f}, z={charge}, range: [{start_mz:.4f}, {end_mz:.4f}], {len(exp_mz_window)} points")
        ...
        if len(compositions) > 0:
            compositions, exp_x0_refined, exp_sigma_refined, has_other_strands, all_compositions, has_odd_n0_warning, _, _, has_unrealistic_n0_warning = analyzer.refine_compositions_with_isotope_matching(
                compositions,
                mz_values,
                intensity_values,
                peak_mz,
        ...
        logger.debug(f"Top {min(10, len(compositions))} compositions after manual fit recalculation (sorted by combined score)")
        ...
        if valid_comps:
            ...
            logger.debug(f"Best X0 match for Qcl filtering: Type={best_comp.get('type')}, nAg={best_comp.get('num_silver')}, N0={best_comp.get('n0')}, Qcl={best_qcl}, X0_error={best_x0_err:.4f}")
            ...
            if best_qcl is not None:
                filtered_compositions = [
                    ...
                    if (comp.get('type') != 'nanocluster') or  # Keep all non-cluster
                       (comp.get('qcl') is not None and abs(comp['qcl'] - best_qcl) <= 1)  # Qcl±1 of best
                ]
                ...
                logger.info(f"FILTERED: Kept {nanocluster_count} nanoclusters (Qcl±1 of {best_qcl}) + {non_cluster_count} non-cluster")
...
def try_higher_strands() -> FlaskResponse:
    ...
    try:
            ...
            for num_ag in range(2, MAX_SILVER + 1):
                ...
                for z_test in z_values:
                    ...
                    for qcl in range(0, num_ag + 1):
                        ...
                        mass = mP_total + mH_total + mC_total + mN_total + mO_total + mAg_total - (qcl + z_test) * analyzer.m_p
        ...
        # Refine with isotope matching
        refined_compositions, exp_x0, exp_sigma, has_other_strands, all_compositions, has_odd_n0_warning, _, _, has_unrealistic_n0_warning = analyzer.refine_compositions_with_isotope_matching(
            compositions,
            mz_values,
            intensity_values,
            peak_mz,
...
def calculate_dna_mass() -> FlaskResponse:
    ...
    try:
        ...
        mass = (analyzer.m_p * nH + analyzer.mC * nC + analyzer.mN * nN +
                analyzer.mO * nO + analyzer.mP * nP)
    ...
    try:
        ...
        for peak in detected_peaks:
            if peak['charge'] is not None:
                spacing_check = analyzer.detect_charge_state(
                    mz_values, intensity_values, peak['mz'], window=3.0
                )
                if (spacing_check['charge'] is not None
                        and spacing_check['num_peaks'] >= 3
                        and spacing_check['charge'] != peak['charge']):
                    logger.info(f"Spacing override at m/z {peak['mz']:.2f}: z={peak['charge']} -> z={spacing_check['charge']}")
...
def download_sample() -> FlaskResponse:
    ...
    try:
        ...
        return send_file(
            sample_file,
            as_attachment=True,
            download_name='sample_spectrum.txt',
            mimetype='text/plain'
        )
...
def find_composition() -> FlaskResponse:
    ...
    try:
        ...
        if formula.endswith('mz'):
            logger.warning(f"Removing 'mz' suffix from formula. Original: '{original_formula}', Cleaned: '{formula[:-2]}'")
            formula = formula[:-2]

        logger.debug(f"Received formula for composition search: '{formula}', charge: {charge}, qcl: {qcl}, adduct_mass: {total_adduct_mass}, adduct_charge: {total_adduct_charge}")
        ...
        result = analyzer.find_composition_in_spectrum(
            formula, charge, qcl, mz_values, intensity_values, peaks_data, resolution,
            adduct_mass=total_adduct_mass, adduct_charge=total_adduct_charge
        )
...
def analyze_region() -> FlaskResponse:
    ...
    try:
        ...
        if detected_charge is not None:
            ...
            if spacing_check['charge'] is not None and spacing_check['num_peaks'] >= 3:
                ...
                if spacing_charge != charge:
                    ...
                    spacing_stronger = (
                        spacing_check['num_peaks'] >= 4
                        and spacing_conf > charge_confidence
                    )
                    if halving_artifact or trusted_supplied or not spacing_stronger:
                        ...
                        logger.info(f"Spacing method disagrees: z={spacing_charge} (from {spacing_check['num_peaks']} peaks, spacing={spacing_check['spacing']:.4f}) vs auto-detected z={charge}")
                        charge = spacing_charge
                        charge_method = 'spacing_override'
                        charge_confidence = max(0.5, spacing_conf)

            logger.info(f"Using charge for peak at m/z {peak_mz:.4f}: z={charge} (method: {charge_method}, confidence: {charge_confidence*100:.1f}%)")
        else:
            ...
            logger.info(f"Final result: z={charge} (method: {charge_method}, confidence: {charge_confidence*100:.1f}%)")

            # If confidence is very low (< 30%), warn user
            if charge_confidence < 0.3:
                logger.warning("Low confidence charge detection! Peak may be weak, noisy, or overlapping. Consider using 'Re-analyze with new z' if results seem incorrect")
        ...
        logger.debug(f"Using DISPLAY window: [{peak_mz - window:.4f}, {peak_mz + window:.4f}] m/z ({len(exp_mz_window)} points)")
        ...
        if len(exp_mz_window) > 0:
            ...
            if exp_mz_gauss is not None and exp_int_gauss is not None and len(exp_mz_gauss) > 3:
                ...
                if is_envelope_flat:
                    logger.debug(f"Envelope too flat (raw_cv={raw_cv*100:.1f}%, envelope_cv={envelope_cv*100:.1f}%) - not displaying envelope or X0")
        ...
        if custom_xna and custom_xna.get('formula'):
            ...
            if xna_mass is None:
                ...
                logger.info(f"Using custom XNA: {custom_xna['name']} (Formula: {custom_xna['formula']}, Calculated Mass: {xna_mass:.2f} Da)")
            else:
                logger.info(f"Using custom XNA: {custom_xna['name']} (Formula: {custom_xna['formula']}, User-Provided Mass: {xna_mass:.2f} Da)")
            ...
            logger.info(f"Complex DNA mode (no XNA formula) - is_complex={custom_xna.get('is_complex')}, using DNA sequence for mass calculation")
        # Use smart adduct search (mass-based filtering, triggered by X₀ error > 0.5)
        compositions = analyzer.analyze_peak_with_smart_adduct_search(
            peak_mz, charge, dna_sequence, exp_x0, resolution=resolution,
            mz_values=mz_values, intensity_values=intensity_values,
            custom_xna=custom_xna,
        )
        ...
        if len(compositions) == 0 and charge is not None:
            ...
            logger.debug(f"Calculated {len(composition_estimates)} composition estimates for user guidance (complex={is_complex})")
        ...
        logger.error(f"ANALYSIS_CRASH in analyze_region: Input={data.get('clicked_mz', 'unknown')}, Error={type(e).__name__}: {str(e)}", exc_info=True)
        return jsonify({'error': str(e)}), 500
...
def smiles_to_formula() -> FlaskResponse:
    ...
    try:
        ...
        try:
            ...
            return jsonify({
                'error': 'RDKit is not installed. Please install it with: pip install rdkit'
            }), 500
...
def isotope_library_settings() -> FlaskResponse:
    ...
    try:
        ...
        if new_library == 'isospec' and not ISOSPEC_AVAILABLE:
            return jsonify({
                'error': 'IsoSpecPy is not installed. Install with: pip install IsoSpecPy'
            }), 400
...
def benchmark_isotope() -> FlaskResponse:
    ...
    try:
        ...
        results['pythoms'] = {
            'time_per_call': pythoms_time,
            'success': 'error' not in pattern_pythoms
        }
        ...
        if ISOSPEC_AVAILABLE:
            ...
            results['isospec'] = {
                'time_per_call': isospec_time,
                'success': 'error' not in pattern_isospec
            }
...
if __name__ == '__main__':
    ...
    if is_port_in_use(port):
        logger.warning(f"Port {port} is already in use! Another server instance may be running. Please stop it first or use a different port.")
        import sys
        sys.exit(1)
    else:
